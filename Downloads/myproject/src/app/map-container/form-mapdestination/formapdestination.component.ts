...
// //  step : 
// 1- récupérer value change des inupt 
// 2 - differencier arrivée et depart et envoyer query pour recuperer res.features qui contient un liste de ville à envoyer à input 
// 3 - faire un ngfor et afficher placename de objet sur input du form 
// 4 - une fois qu'on a choisi option on récupere place object qui  continent toutes infos ( coord , center , placename) au ts
// 5- il disting si arrivee ou depart pour recuperer LngLat et la stocker dans une variable departlatlong = event.center  . 
//6 - une fois que j'ai coord des 2 inputs je submit appelle mapDestinationSubmit() elle va gérer boolean d'affichage des composants 
 // et aussi faire un out put de ces 2 coordonnées au parent (container ) pour les envoyer à la map (enfant). 